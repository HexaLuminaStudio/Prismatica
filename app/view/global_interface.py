...
class GlobalInterface(QWidget):
    ...
    def _onBatchAddClicked(self) -> None:
        ...
        dialog = BatchDownloadDialog("Global", infoDict, self.window())
        dialog.exec()
    ...
    def _onBatchDownloadClicked(self) -> None:
        ...
        batchApplyService.clearAll(GLOBAL_DOWNLOAD_TASK_TYPE)
        if created > 0:
            InfoBar.success(
                title="批量任务已创建",
    ...
    def _onBatchItemsChanged(self, _count: int) -> None:
        """刷新当前来源的批量清单状态。"""
        count = batchApplyService.getCount(GLOBAL_DOWNLOAD_TASK_TYPE)
