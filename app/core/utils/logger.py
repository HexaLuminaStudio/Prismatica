...
SENSITIVE_PATTERNS_LIST: List[Pattern] = [
    ...
    re.compile(r'(secret[_-]?key|secret[_-]?token|bearer\s+)[\w\-]{16,}', re.IGNORECASE),
    ...
    # 认证信息
    re.compile(r'(authorization|bearer|token|jwt)[\s:=]+["\']?[A-Za-z0-9\-_.~+/]+=*["\']?', re.IGNORECASE),
    ...
    # 数据库连接字符串
    re.compile(r'(mongodb|mysql|postgresql|redis):\/\/[^\s]+:[^\s]+@[^\s]+', re.IGNORECASE),
    re.compile(r'(host|server|database|db)[_-]?(name|user)[\s:=]+[^\s,;]+', re.IGNORECASE),
    
    # SSH密钥和私钥
    re.compile(r'-----BEGIN\s+(RSA|DSA|EC|OPENSSH)?\s*PRIVATE\s+KEY-----'),
    
    # 财务数据
    re.compile(r'(account[_-]?no|bank[_-]?card|credit[_-]?card|银行卡)[:s:=：]+\d+', re.IGNORECASE),
...
def _filterSensitiveInfo(message: str) -> str:
    ...
    for pattern in SENSITIVE_PATTERNS_LIST:
        filteredMessage = pattern.sub(lambda m: _maskSensitiveValue(m.group()), filteredMessage)
